...
contract Treasury is OwnableUpgradeable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SUDO }
    ...
    address public sUDO;
    uint public sUDOQueue; // Delays change to sUDO address
    ...
    /**
        @notice allow approved address to deposit an asset for UDO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint UDO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( UDO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn UDO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IUDOERC20( UDO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sUDO ).balanceOf( msg.sender ); // Can only borrow against sUDO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with UDO
        @param _amount uint
     */
    function repayDebtWithUDO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IUDOERC20( UDO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, UDO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( UDO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( UDO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns UDO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match UDO decimals
            value_ = _amount.mul( 10 ** IERC20( UDO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SUDO ) { // 9
            sUDOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
